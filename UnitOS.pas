...
begin
              if StrToDateTime(EditDataFinal.Text) < StrToDateTime(EditData.Text) then
                    begin
                           MessageBox(0, 'Data de fim da vigência não pode ser anterior ao inicio.'+#13+#10+''+#13+#10+'Corrija', 'Erro na informação de datas', MB_ICONSTOP or MB_OK);
                           Abort;
              ...
              if DBLookupComboBox1.KeyValue = NULL then
                   BEGIN
                         MessageBox(0, 'É necessário informar o tipo de OS para abertura', 'incompleto', MB_ICONSTOP or MB_OK);
                         abort;
                   END;
              if memo1.Text = '' then
                  begin
                        MessageBox(0, 'O campo de informações complementares é obrigatório', 'Falta Preenchimento', MB_ICONWARNING or MB_OK);
                        Abort;
                  end;
              ...
              // VEICULO É PREENCHIDO NA CHAMADA DO METODO FORMOS, PASSA PARAMETRO VOS_VEI
