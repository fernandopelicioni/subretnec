...
begin
//Devo colocar restrição de senhas aqui
Screen.Cursor:=Crhourglass;
if EditColigada.Text = '' then
    begin
    MessageBox(Handle,'Você precisa selecionar uma coligada !!!                    '+#13+#13+#13+'Tente novamente !','Atenção !!!', MB_OK + MB_OK);
    Edit1.SetFocus;
    end
    Else
    begin
                          ...
                          if (DtUsuarios.DataSet.FieldByName('usu_Perfil').Text = DtColigada.DataSet.FieldByName('Perfil').text) or (DtUsuarios.DataSet.FieldByName('usu_Perfil').Text = 'T')  then
                             Begin
                                 ...
                                 // Application.CreateForm(TFormEntrada,FormEntrada);
                                  ...
                                  FormNucleo.Panel3.Caption:= 'Usuário : ' + Pai.QrUsuarios.FieldByName('Usu_Nome').AsString + ' - ' + FormIniciaSistema.editversao.Caption + ' | Coligada : ' + Pai.QrColigadas.FieldByName('Des_Coligada').AsString;
...
//                                               FormEntrada.Ribbon1.Style:= RibbonLunaStyle;
                             ...
                             End
                             ELSE BEGIN
                                  MessageBox(Handle,'Você não esta autorizado para Logar nesta Coligada, Selecione Outra !','Acesso Negado',Mb_IconInformation+Mb_Ok);
    ...
    if key = #13 then
           begin
               ...
               if editcoligada.KeyValue = Null then
                     begin
                          ShowMessage('Coligada não selecionada !');
                     end
                     else
                     BitBtn1Click(nil);
   end;
