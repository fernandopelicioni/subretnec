...
//                ShowMessage('MovPrd');
  ...
  begin
    MessageBox(Handle,'Veículo não esta Cadastrado, Indique Outro, por favor','Não Encontrado',Mb_iconInformation+Mb_ok);
...
if StrToInt(KmRod.Caption) > StrToInt(DtBaseRevisao.DataSet.FieldByName('Sco_Validade').AsString) then
     ...
     Else
     label15.Caption:='Item NÂO está Vencido.';
...
if FormNucleo.nivel > 9 then
     begin
    if MessageDlg('Excluir um serviço Controlado pode refletir negativamente na manutenção do veículo, você precisa estar certo disto'+#13+#13+#13+'Deseja mesmo Continuar ? +' ,mtConfirmation,[MBYES,MBNO],0) = MRYES then
         begin
               With QrTemp do
                    begin
                           close;
...
begin
            ...
            if OsStatus = 'A' then
                     begin
                               MessageBox(0, 'Encontrei uma OS aberta/pendente'+#13+#10+''+#13+#10+'Será necessário fecha-la para abrir outra!', 'Ordem de Serviço Aberta', MB_ICONINFORMATION or MB_OK);
                               abort;
                     end
                     else
                     begin
                              ...
                              PAI.VOS_HIS:='OS gerada a partir da Revisão Preventiva';
                              PAI.VOS_CODMTV:=3;
...
With Pai.QrMovRevisao do
       ...
       FormLancaNovaRevisao.label2.Caption:='Revisão Preventiva';
              FormLancaNovaRevisao.ShowModal;
       //showMessage(numOs);
       DataRevisao:=DtRevGer.DataSet.FieldByName('TRG_Data').AsString;

       if MessageDlg('Deseja Continuar com a emissão da Nova Ordem de Revisão : ' + NumOS + #13+#13 + 'Para Continuar confirme !',MtInformation,[mbyes,mbno],0) = Mryes then
           begin
...
//                              QrPmc.Parameters.ParamByName('veiculo').
                              ...
                              if not QrPmc.eof then
                                     Relatorio.Projeto.SetParam('Obs','* Atenção, Veículo com Defeito Reclamado, Consulte !')
                                     ELSE
...
//                              Relatorio.Projeto.close;
   ...
   Screen.Cursor:=CrDefault;
    ...
    begin
      MessageBox(handle,'Existe Revisões ou Itens das Revisões Abertas, É necessário fecha-los antes de Emitir nova Ordem de Serviço !!','Revisões não Finalizadas',MB_IconInformation+mb_Ok);
    end;
end;
...
//                      Sql.Add('and C.Vei_Coligada = B.Sco_Coligada');
                   ...
                   if not eof then
                      ...
                      Else begin
                        MessageBox(Handle,'Contate o Desenvolvedor do Software. Há alguma inconsistencia...','Atenção',Mb_IconInformation+mb_ok);
...
begin
      ...
      DtTbMovRev.DataSet.FieldByName('RPR_His').AsString:='Este item não foi revisado nesta data, antes do vencimento, ou desnecessário, em : ' + DateTimeToStr(now);
      DtTbMovRev.DataSet.post;
      DtTbBaseRevisao.DataSet.Post;
      DtMovRevPre.DataSet.Close;
      DtMovRevPre.DataSet.open;
          ...
          { // VerificaOSpendente;
            ...
            if OsStatus = 'A' then
                     ...
                     begin
                              ...
                              Pai.VOS_DTABE:=(now);
                              pai.VOS_DTFEC:=StrToDate('01/01/1900');
                              PAI.VOS_HIS:='OS gerada a partir da Revisão Corretiva';
                              PAI.VOS_CODMTV:=4;
                              Pai.VOS_DTPrazo:=(now+2);
                              pai.VOS_HRABE:=StrToTime(copy(TimeToStr(now),1,5));
                              pai.VOS_HRFEC:=strtotime('00:00');
...
With Pai.QrMovRevisao do
       ...
       FormLancaNovaRevisao.label2.Caption:='Revisão Corretiva';
       FormLancaNovaRevisao.ShowModal;
       DataRevisao:=DtRevGer.DataSet.FieldByName('TRg_Data').AsString;
       //showMessage(numOs);
       if MessageDlg('Revisão Corretiva' +#13+#13+'Deseja Continuar com a emissão da Nova Ordem de Revisão : ' + NumOS + #13+#13 + 'Para Continuar confirme !',MtInformation,[mbyes,mbno],0) = Mryes then
           begin
           //   DtRevGer.DataSet.FieldByName('TRG_DATA').AsString:=DateToStr(now);
                DtRevGer.DataSet.Post;
          ...
          with Relatorio.QrBaseRev do
               ...
               if not eof then
                 ...
                 with Relatorio.QrBaseRev do
                   ...
                   if not eof then
                      begin
                           if MessageDlg('A Revisão foi gerada com Sucesso, Deseja imprimi-la ?',mtConfirmation,[mbyes,mbno],0) = mryes  then
                                begin
                                     Relatorio.Projeto.SetParam('Dt1',DataRevisao);
                                     Relatorio.Projeto.SetParam('Versao',FormIniciaSistema.EditVersao.Caption);
                                     Relatorio.Projeto.ExecuteReport('RvFichaVistoria');
...
begin
/// RECEBER TODAS REVISOES DE UMA VEZ SEM ALTERAÇÃO
///
///

      if (MessageDlg('Este procedimento recebe todas os itens da revisão sem alterações, você só deve fazer isso após receber os itens que sofrerão alterações'+#13+#10+''+#13+#10+'Deseja continuar?', mtInformation, [mbYes, mbNo], 0) = mrYes) then
      begin
            Screen.Cursor:=CrHourGlass;
            ...
            With QrAtualizaMov do
                 begin
                       ...
                       Sql.Clear;
                         ...
                         Parameters.ParamByName('his').Value:='Não revisao ou desnecessário - ' + DateToStr(now);
                         ExecSql;
                 end;
