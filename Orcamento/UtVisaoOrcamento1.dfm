object FormVisaoOrcamento1: TFormVisaoOrcamento1
  Left = 0
  Top = 0
  BorderIcons = []
  Caption = 'Genoma Software'
  ClientHeight = 610
  ClientWidth = 1084
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -19
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Touch.GestureManager = GestureManager1
  Touch.InteractiveGestureOptions = [igoPanSingleFingerHorizontal, igoPanInertia, igoParentPassthrough]
  OnGesture = FormGesture
  OnKeyDown = FormKeyDown
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 25
  object AppBar: TPanel
    Left = 0
    Top = 855
    Width = 1340
    Height = 75
    BevelOuter = bvNone
    TabOrder = 0
    Visible = False
    object CloseButton: TImage
      Left = 1280
      Top = 0
      Width = 60
      Height = 75
      Align = alRight
      Center = True
      Picture.Data = {
        
        7F018E704185241CEA460000000049454E44AE426082}
      OnClick = CloseButtonClick
      ExplicitLeft = 1936
    end
  end
  object Panel1: TPanel
    AlignWithMargins = True
    Left = 3
    Top = 3
    Width = 1078
    Height = 55
    Align = alTop
    Alignment = taLeftJustify
    Caption = '   Vis'#227'o de contas or'#231'amentarias | Abril de 2017'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Segoe UI Light'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    ExplicitWidth = 1033
    object Image1: TImage
      Left = 1027
      Top = 1
      Width = 50
      Height = 53
      Cursor = crHandPoint
      Hint = 'Sair'
      Align = alRight
      AutoSize = True
      ParentShowHint = False
      Picture.Data = {
        
        FF7EB4DE733B0A549E0000000049454E44AE426082}
      ShowHint = True
      OnClick = Image1Click
      ExplicitLeft = 748
      ExplicitTop = 2
      ExplicitHeight = 63
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 61
    Width = 1084
    Height = 549
    Align = alClient
    DataSource = DtContasOrcamentarias
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Segoe UI'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -19
    TitleFont.Name = 'Segoe UI'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    Columns = <
      item
        Expanded = False
        FieldName = 'CODTBORCAMENTO'
        Title.Caption = 'Conta'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Segoe UI'
        Title.Font.Style = []
        Width = 147
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESCRICAO'
        Title.Caption = 'Descri'#231#227'o'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Segoe UI'
        Title.Font.Style = []
        Width = 497
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ORCado'
        Title.Caption = 'Valor Or'#231'ado'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Segoe UI'
        Title.Font.Style = []
        Width = 130
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'REALIZADO'
        Title.Caption = 'Valor Realizado'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Segoe UI'
        Title.Font.Style = []
        Width = 130
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESVIO'
        Title.Caption = 'Desvio'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -16
        Title.Font.Name = 'Segoe UI'
        Title.Font.Style = []
        Width = 123
        Visible = True
      end>
  end
  object ActionList1: TActionList
    Left = 192
    Top = 248
    object Action1: TAction
      Caption = 'Action1'
      OnExecute = Action1Execute
    end
  end
  object GestureManager1: TGestureManager
    Left = 120
    Top = 248
    GestureData = <
      item
        Control = Owner
        Collection = <
          item
            Action = Action1
            GestureID = sgiUp
          end>
      end>
  end
  object QrJan: TFDQuery
    Connection = pai.FdCorpore
    FormatOptions.AssignedValues = [fvFmtDisplayNumeric]
    FormatOptions.FmtDisplayNumeric = '0.00'
    SQL.Strings = (
      ''
      'SELECT '
      'iorc.SINTETICOANALITICO,'
      'IORC.CODTBORCAMENTO, '
      'IORC.DESCRICAO,'
      ''
      '-- ********** ABRIL ******************************************'
      #9'ITEORCABR.VALORORCADO AS ORCado,'
      #9
      
        #9'(SELECT SUM(TT.VALORLIQUIDO) FROM TMOV T JOIN TITMMOV TT ON T.C' +
        'ODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = TT.IDMOV WHERE T.CODTM' +
        'V LIKE '#39'1.2.%'#39' AND T.DATAEMISSAO >= ITABR.DATAINICIO AND T.DATAE' +
        'MISSAO <= ITABR.DATAFIM AND TT.CODTBORCAMENTO = IORC.CODTBORCAME' +
        'NTO AND TT.CODCOLIGADA = IORC.CODCOLIGADA) AS REALIZADO,'
      
        #9'ITEORCABR.VALORORCADO - (SELECT SUM(TT.VALORLIQUIDO) FROM TMOV ' +
        'T JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV ' +
        '= TT.IDMOV WHERE T.CODTMV LIKE '#39'1.2.%'#39' AND T.DATAEMISSAO >= ITAB' +
        'R.DATAINICIO AND T.DATAEMISSAO <= ITABR.DATAFIM AND TT.CODTBORCA' +
        'MENTO = IORC.CODTBORCAMENTO AND TT.CODCOLIGADA = IORC.CODCOLIGAD' +
        'A) as DESVIO'
      ''
      ''
      'FROM TTBORCAMENTO IORC'
      ''
      
        ' left  JOIN TORCAMENTO ORC ON ORC.CODCOLIGADA = IORC.CODCOLIGADA' +
        ' AND ORC.CODTBORCAMENTO = IORC.CODTBORCAMENTO'
      ' '
      '--********************ABRIL'
      
        ' left JOIN TITMPERIODOORCAMENTO ITABR ON ITABR.CODCOLIGADA = IOR' +
        'C.CODCOLIGADA  AND ITABR.IDITMPERIODO=4 AND YEAR(ITABR.DATAINICI' +
        'O)=2017'
      
        ' left JOIN TITMORCAMENTO ITEORCABR ON ITEORCABR.CODCOLIGADA = IO' +
        'RC.CODCOLIGADA AND ITEORCABR.IDORCAMENTO = ORC.IDORCAMENTO AND I' +
        'TEORCABR.IDITMPERIODO = ITABR.IDITMPERIODO'
      ''
      ' '
      ' WHERE IORC.CODCOLIGADA =:COLIGADA'
      ''
      ' ORDER BY IORC.CODTBORCAMENTO')
    Left = 720
    Top = 424
    ParamData = <
      item
        Name = 'COLIGADA'
        DataType = ftString
        ParamType = ptInput
        Value = '1'
      end>
  end
  object DtContasOrcamentarias: TDataSource
    DataSet = QrJan
    Left = 720
    Top = 480
  end
end
