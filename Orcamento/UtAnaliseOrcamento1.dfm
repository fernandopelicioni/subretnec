object FormAnaliseOrcamento1: TFormAnaliseOrcamento1
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'FormAnaliseOrcamento1'
  ClientHeight = 930
  ClientWidth = 1340
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clMaroon
  Font.Height = -19
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Touch.GestureManager = GestureManager1
  Touch.InteractiveGestureOptions = [igoPanSingleFingerHorizontal, igoPanInertia, igoParentPassthrough]
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnGesture = FormGesture
  OnKeyDown = FormKeyDown
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 25
  object Label4: TLabel
    Left = 288
    Top = 40
    Width = 55
    Height = 25
    Caption = 'Label4'
  end
  object Label6: TLabel
    Left = 98
    Top = 528
    Width = 88
    Height = 25
    Caption = 'Corlaranja'
    Color = 3951847
    ParentColor = False
    Transparent = False
    Visible = False
  end
  object AppBar: TPanel
    Left = 0
    Top = 855
    Width = 1340
    Height = 75
    BevelOuter = bvNone
    TabOrder = 0
    Visible = False
    object CloseButton: TImage
      Left = 1280
      Top = 0
      Width = 60
      Height = 75
      Align = alRight
      Center = True
      Picture.Data = {
        
        7F018E704185241CEA460000000049454E44AE426082}
      OnClick = CloseButtonClick
      ExplicitLeft = 1936
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1340
    Height = 105
    Align = alTop
    Alignment = taRightJustify
    Caption = 'Monitor de Or'#231'amento mensal         '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -19
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    VerticalAlignment = taAlignTop
    object ToolBar1: TToolBar
      Left = 1
      Top = 46
      Width = 1338
      Height = 58
      Align = alBottom
      ButtonHeight = 54
      ButtonWidth = 55
      Caption = 'ToolBar1'
      Color = clNone
      Images = ImageList1
      ParentColor = False
      TabOrder = 0
      Transparent = True
      object ToolButton3: TToolButton
        Left = 0
        Top = 0
        Width = 12
        Caption = 'ToolButton3'
        ImageIndex = 12
        Style = tbsSeparator
      end
      object ToolButton1: TToolButton
        Left = 12
        Top = 0
        Caption = 'ToolButton1'
        ImageIndex = 2
        OnClick = ToolButton1Click
      end
      object ToolButton4: TToolButton
        Left = 67
        Top = 0
        Caption = 'ToolButton4'
        ImageIndex = 4
        OnClick = ToolButton4Click
      end
    end
  end
  object FlowPanel1: TFlowPanel
    AlignWithMargins = True
    Left = 50
    Top = 108
    Width = 1240
    Height = 819
    Margins.Left = 50
    Margins.Right = 50
    Align = alClient
    TabOrder = 2
    object PnDirecao: TJvCaptionPanel
      Left = 1
      Top = 1
      Width = 292
      Height = 336
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Dire'#231#227'o'
      CaptionColor = clBlack
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 0
      object LbDirecao: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '68%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 127
      end
      object Chart2: TChart
        Left = 1
        Top = 104
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart2Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries1: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          SeriesColor = 7457838
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            0602000000000000000068AC4001A6E800FF060000004F72E761646F00000000
            00C1B0402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnEletrica: TJvCaptionPanel
      Left = 293
      Top = 1
      Width = 292
      Height = 336
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'El'#233'trica'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 2
      object LbEletrica: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '125%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart1: TChart
        Left = 1
        Top = 104
        Width = 290
        Height = 231
        Cursor = crHandPoint
        Border.EndStyle = esFlat
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        MarginLeft = 5
        MarginRight = 5
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart1Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object Series1: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            060200000000000000001FE74001A6E800FF060000004F72E761646F00000000
            207AF2402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnFreios: TJvCaptionPanel
      Left = 585
      Top = 1
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Freios'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 1
      object LbFreios: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart3: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart3Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries2: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnMotor: TJvCaptionPanel
      Left = 877
      Top = 1
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Motor'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 3
      object LbMotor: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart4: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart4Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries3: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnLanternagem: TJvCaptionPanel
      Left = 1
      Top = 337
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Lanternagem'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 4
      object Lblanternagem: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart5: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart5Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries4: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnSuspensao: TJvCaptionPanel
      Left = 293
      Top = 337
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Suspens'#227'o'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 5
      object LbSuspensao: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart6: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart6Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries5: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnTransmissao: TJvCaptionPanel
      Left = 585
      Top = 337
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Transmiss'#227'o'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 6
      object LbTransmissao: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart7: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart7Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries6: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
    object PnLimpeza: TJvCaptionPanel
      Left = 877
      Top = 337
      Width = 292
      Height = 335
      AutoDrag = False
      Buttons = []
      BorderStyle = bsNone
      Caption = 'Limpeza'
      CaptionColor = 15246849
      CaptionPosition = dpTop
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWhite
      CaptionFont.Height = -13
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = [fsBold]
      FlatButtons = True
      OutlookLook = False
      TabOrder = 7
      object LbLimpeza: TLabel
        AlignWithMargins = True
        Left = 4
        Top = 28
        Width = 284
        Height = 89
        Align = alTop
        Alignment = taCenter
        Caption = '108%'
        Color = 15246849
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -67
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        Layout = tlBottom
        ExplicitWidth = 163
      end
      object Chart8: TChart
        Left = 1
        Top = 103
        Width = 290
        Height = 231
        Cursor = crHandPoint
        LeftWall.Brush.Style = bsVertical
        LeftWall.Pen.Visible = False
        LeftWall.Visible = False
        Legend.Alignment = laBottom
        Title.Margins.Units = maPixels
        Title.Text.Strings = (
          'Or'#231'amento')
        AxisBehind = False
        AxisVisible = False
        BottomAxis.Title.Visible = False
        DepthAxis.Title.Visible = False
        DepthTopAxis.Title.Visible = False
        Hover.Visible = False
        LeftAxis.Title.Visible = False
        Panning.MouseWheel = pmwNone
        RightAxis.Title.Visible = False
        TopAxis.Title.Visible = False
        Zoom.Pen.Visible = False
        Align = alBottom
        TabOrder = 0
        OnClick = Chart8Click
        DefaultCanvas = 'TGDIPlusCanvas'
        PrintMargins = (
          15
          18
          15
          18)
        ColorPaletteIndex = 13
        object BarSeries7: TBarSeries
          Legend.Visible = False
          BarPen.EndStyle = esFlat
          BarPen.Visible = False
          Marks.Frame.Visible = False
          Marks.Style = smsLabelValue
          ShowInLegend = False
          XValues.Name = 'X'
          XValues.Order = loAscending
          YValues.Name = 'Bar'
          YValues.Order = loNone
          Data = {
            06020000000000000000DED54001A6E800FF060000004F72E761646F00000000
            4002DA402ECC7100FF090000005265616C697A61646F}
          Detail = {0000000000}
        end
      end
    end
  end
  object ActionList1: TActionList
    Left = 216
    Top = 24
    object Action1: TAction
      Caption = 'Action1'
      OnExecute = Action1Execute
    end
  end
  object GestureManager1: TGestureManager
    Left = 24
    Top = 280
    GestureData = <
      item
        Control = Owner
        Collection = <
          item
            Action = Action1
            GestureID = sgiUp
          end>
      end>
  end
  object QrOrc1: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      ''
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.004'#39')'
      'ORDER BY COD')
    Left = 504
    Top = 264
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtOrc1: TDataSource
    DataSet = QrOrc1
    Left = 505
    Top = 312
  end
  object DBCrossTabSource1: TDBCrossTabSource
    GroupField = 'VALORORCADO'
    LabelField = 'DESCRICAO'
    ValueField = 'VALORORCADO'
    DataSet = QrOrc1
    Left = 16
    Top = 216
  end
  object ImageList1: TImageList
    ColorDepth = cd32Bit
    DrawingStyle = dsTransparent
    Height = 48
    Width = 48
    Left = 240
    Top = 560
    Bitmap = {
      
      000000000000}
  end
  object Timer1: TTimer
    Interval = 60000
    OnTimer = Timer1Timer
    Left = 538
    Top = 564
  end
  object QrDirecao: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      ''
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39')'
      'ORDER BY COD')
    Left = 224
    Top = 304
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtDirecao: TDataSource
    DataSet = QrDirecao
    Left = 225
    Top = 352
  end
  object QrFreios: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.005'#39')'
      'ORDER BY COD')
    Left = 768
    Top = 280
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtFerios: TDataSource
    DataSet = QrFreios
    Left = 769
    Top = 328
  end
  object QrMotor: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.007'#39')'
      'ORDER BY COD')
    Left = 1104
    Top = 256
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtMotor: TDataSource
    DataSet = QrMotor
    Left = 1105
    Top = 304
  end
  object QrLanternagem: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.006'#39')'
      'ORDER BY COD')
    Left = 128
    Top = 544
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object Dtlanternagem: TDataSource
    DataSet = QrLanternagem
    Left = 129
    Top = 592
  end
  object QrSuspensao: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        ' AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DA' +
        'TAFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM T' +
        'ORCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.008'#39')'
      'ORDER BY COD')
    Left = 464
    Top = 552
    ParamData = <
      item
        Name = 'MES'
        DataType = ftString
        ParamType = ptInput
        Value = '8'
      end
      item
        Name = 'ANO'
        DataType = ftString
        ParamType = ptInput
        Value = '2017'
      end>
  end
  object DtSuspensao: TDataSource
    DataSet = QrSuspensao
    Left = 465
    Top = 600
  end
  object QrTransmissao: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.009'#39')'
      'ORDER BY COD')
    Left = 776
    Top = 560
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtTransmissao: TDataSource
    DataSet = QrTransmissao
    Left = 777
    Top = 608
  end
  object QrLimpeza: TFDQuery
    Connection = pai.FdCorpore
    SQL.Strings = (
      
        'SELECT TTB.CODTBORCAMENTO COD, TTB.DESCRICAO,TTOC.VALORORCADO Or' +
        'cado, month(TTP.DATAINICIO) Mes, (SELECT SUM(TT.VALORLIQUIDO) FR' +
        'OM TMOV T '
      
        'JOIN TITMMOV TT ON T.CODCOLIGADA = TT.CODCOLIGADA AND T.IDMOV = ' +
        'TT.IDMOV'
      #9#9
      'WHERE T.CODTMV LIKE '#39'1.2.%'#39' '
      
        'AND T.CODTMV NOT IN ('#39'1.2.10'#39','#39'1.2.11'#39','#39'1.2.18'#39','#39'1.2.19'#39','#39'1.2.21' +
        #39','#39'1.2.50'#39')'
      
        'AND T.DATAEMISSAO >= TTP.DATAINICIO AND T.DATAEMISSAO <= TTP.DAT' +
        'AFIM AND TT.CODTBORCAMENTO = TOC.CODTBORCAMENTO) AS REAL FROM TO' +
        'RCAMENTO TOC'
      ''
      
        'JOIN TITMORCAMENTO TTOC ON  TOC.CODCOLIGADA = TTOC.CODCOLIGADA A' +
        'ND  TOC.IDORCAMENTO = TTOC.IDORCAMENTO'
      
        'JOIN TITMPERIODOORCAMENTO TTP ON TTOC.CODCOLIGADA = TTP.CODCOLIG' +
        'ADA AND TTOC.IDPERIODO = TTP.IDPERIODO AND TTOC.IDITMPERIODO = T' +
        'TP.IDITMPERIODO '
      
        'JOIN TTBORCAMENTO TTB ON TOC.CODCOLIGADA = TTB.CODCOLIGADA AND T' +
        'OC.CODTBORCAMENTO = TTB.CODTBORCAMENTO'
      ''
      ''
      'WHERE MONTH(TTP.DATAINICIO) =:mes AND YEAR(TTP.DATAINICIO) =:ano'
      
        '--WHERE TTP.DATAINICIO>= '#39'01/07/2017'#39' AND TTP.DATAFIM <='#39'31/07/2' +
        '017'#39
      'AND TTOC.CODCOLIGADA=1'
      ''
      
        '--AND TTB.CODTBORCAMENTO in ('#39'2.1.01.003'#39', '#39'2.1.01.004'#39','#39'2.1.01.' +
        '005'#39','#39'2.1.01.006'#39','#39'2.1.01.007'#39','#39'2.1.01.008'#39','#39'2.1.01.009'#39','#39'2.1.01' +
        '.025'#39' )'
      'AND TTB.CODTBORCAMENTO in ('#39'2.1.01.025'#39')'
      'ORDER BY COD')
    Left = 1008
    Top = 560
    ParamData = <
      item
        Name = 'MES'
        ParamType = ptInput
      end
      item
        Name = 'ANO'
        ParamType = ptInput
      end>
  end
  object DtLimpza: TDataSource
    DataSet = QrLimpeza
    Left = 1009
    Top = 608
  end
end
