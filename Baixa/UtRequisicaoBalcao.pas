       ...
       begin
             if TestaSaldo(DtRequisicoes.DataSet.FieldByName('Saldo').AsInteger,DtRequisicoes.DataSet.FieldByName('Qtd').AsInteger ) then
                 begin
                    ...
                    MessageBox(Handle,'Saldo do produto zerado ou é menor que a quantidade de retirada, informe ao setor de compras!','Erro de saldo',Mb_IconInformation + Mb_Ok);

       end
       else
        MessageBox(Handle,'Senha informada não confere, produto não foi baixado!','Erro de senha',Mb_IconInformation + Mb_Ok);
        atualizareq;
end;
...
begin
      Panel2.Caption:='                        Baixa em requisições de produtos - ' + DateToStr(now);
      EditUsuario.Caption:='Usuário: ' + pai.VUSUARIO;
