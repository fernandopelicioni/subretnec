...
if (nivel > 9) then
    begin
    MessageBox(Handle,'O Que você esta prestes a fazer é perigoso, tenha atenção aos índices que irá alterar','Atenção !!',MB_IconInformation + MB_Ok);
    Application.CreateForm(TFormIndiceAltera,FormIndiceAltera);
    FormIndiceAltera.ShowModal;
    end
    Else
    MessageBox(Handle,'Desculpe, Mas essa operação requer elevação para ser executada','Aviso !!',MB_IconInformation + MB_Ok);
...
if nivel > 49 then
    ...
    Else
    MessageBox(Handle,'Operação requer nível Alfa','Aviso !!',MB_IconStop + MB_Ok);
...
Statusbar1.Panels[0].Text:='   Usuário : ' + FormIniciaSistema.Dtusuarios.DataSet.FieldByName('Usu_login').asString;
...
label4.Caption:='Você está logado em : ' + Pai.QrColigadas.FieldByName('Des_Coligada').AsString;
