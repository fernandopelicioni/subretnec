...
With Pai.QrMovAbast do
  ...
  if eof then
    ...
    ShowMessage('Já existe um lançamento para este veículo !, Se Deseja alterar, apague o lançamento');
    DBLookupComboBox1.Setfocus;
...
//                DBLookupComboBox1.Setfocus;
       ...
       Else
       Begin
         MessageBox(Handle,'Veículo Não Encontrado','Não Achei',Mb_IconInformation+MB_Ok);
         DBLookupComboBox1.SetFocus;
       End;
...
//           SHOWMESSAGE(IntToStr(KmRodado)+', rodados, '+IntToStr(KmAnterior)+' Anterior');
            if StrtoInt(EditKm.Text) < KmAnterior then
                  begin
                        MessageBox(handle,'Odometro informado é menor do que o anterior, verifique','Erro Encontrado',Mb_IconInformation + Mb_ok);
                        EditKm.Text:='';
                        DBLookupComboBox1.Setfocus;
                  end
                  else
                  begin
                        if KmRodado > DtVeiculo.DataSet.FieldByName('Mod_medKm').AsInteger then
                             begin
                                     ShowMessage('Quantidade de Quilometros rodados do dia excede Limete dos Parâmetros'+#13+
                                     IntToStr(Kmrodado) + ', Limete é de: ' + DtVeiculo.DataSet.FieldByName('Mod_medKm').AsString);
                                     EditKm.SetFocus;
                              end
                              else
                              begin
     ...
     begin  //
        ...
        Label15.Caption:=FormatFloat('##0.00',Media);
         ...
         if (Media >= StrtoFloat(DtVeiculo.DataSet.FieldByName('Mod_MedCombustivel').AsString)) and  (Media <= StrtoFloat(DtVeiculo.DataSet.FieldByName('Mod_MedComMax').AsString)) then
            ...
            Else
              begin //
                 if MessageDlg('Média esta fora dos limites estabelecidos pelo controlador'+#13+#13+ FormatFloat('##0.00',Media) + #13+ 'Foi erro de Lançamento ?',MtInformation,[mbYes,MbNo], 0) = Mrno then
                     begin  //
                    if(StrToInt(EditKm.Text) > StrToInt(DtVeiculo.DataSet.FieldByName('TIn_Odo').AsString)) or (troca = 'S') then
                        begin //
                                DtIndiceRevisao.DataSet.FieldByName('TIN_ODo').AsString:=EditKm.Text;
...
//Dg:string;
begin
Kmanterior:=StrtoInt(InputBox('Alteração de Odometro','Digite o Novo Odometro: ',''));
      try
            Troca:='S';
      except
