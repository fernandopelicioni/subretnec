...
begin
if (MessageDlg('Confirma?'+#13+#10+''+#13+#10+'Observe que ao fechar, não será mais possivel baixar produtos para esse veículo até que uma nova OS seja aberta!'+#13+#10+''+#13+#10+'Esta operação não pode ser desfeita', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
    begin
...
//        Imprimi OS apenas se tiver dentro da vigencia
                         ...
                         if DtMovOSCompleto.DataSet.FieldByName('Prt').AsString = '1' then
                              MessageBox(0, 'Atenção'+#13+#10+''+#13+#10+'Já me consta que essa Ordem de Serviço foi impressa recentemente.'+#13+#10+''+#13+#10+'Só imprima novamente em caso de extravio!', 'OS Já Impressa!', MB_ICONWARNING or MB_OK);
                         ...
                         with Relatorio.QrOsMov do
                              begin
                                  ...
                                  if Not Eof then
                                        ...
                                        MessageBox(0, 'Não existe OS pendente para impressão', 'Não há dados', MB_ICONINFORMATION or MB_OK);
